...
const RequestUserContent = () => {
  ...
  useEffect(() => {
    ...
    const savedHiddenRequests = JSON.parse(localStorage.getItem('hiddenRequests')) || [];
    const savedShowHidden = JSON.parse(localStorage.getItem('showHidden')) || false;
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="text-left text-gray-600 uppercase text-xs md:text-sm">
              {[{ label: "Sr. No", key: null }, { label: "Date", key: "requestDate" }, { label: "Requestor Name", key: "requestorName" }, { label: "Method", key: "communicationMethod" }, { label: "Description", key: "description" }, { label: "Status", key: "priority" }, { label: "Action", key: null }].map(({ label, key }, index) => (
                <th
                  key={index}
          ...
          <tbody>
            {currentRequests.map((request, index) => (
              <tr key={request.id} className="border-b hover:bg-gray-50 text-xs md:text-sm">
                <td className="py-3 px-2 md:px-4">
                  {indexOfFirstRequest + index + 1}
                </td>
                <td className="py-3 px-2 md:px-4">
                  {request?.requestDate ? request?.requestDate.split("T")[0] : "-"}
                </td>
                <td className="py-3 px-2 md:px-4">
